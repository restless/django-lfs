...
urlpatterns += patterns('lfs.catalog.views',
    url(r'^category-(?P<slug>[-\w]+)-i(?P<category_id>\d+)$', "category_view", name="lfs_category"),
    url(r'^product/(?P<slug>[-\w]+)-i(?P<product_id>\d+)$', "product_view", name="lfs_product"),
    ...
    url(r'^set-product-filter/(?P<category_id>[-\w]+)/(?P<property_id>\d+)/(?P<min>.+)/(?P<max>.+)', "set_filter", name="lfs_set_product_filter"),

...
def set_filter(request, category_id, property_group_id, property_id, value=None, min=None, max=None):
...
def reset_filter(request, category_id, property_group_id, property_id):
...
def reset_number_filter(request, category_id, property_group_id, property_id):
    """Resets product filter with given property id. Redirects to the category
    with given slug.
    """
    key = '{0}_{1}'.format(property_group_id, property_id)
...
def category_view(request, slug, category_id, template_name="lfs/catalog/category_base.html"):
    ...
    if category.get_content() == CONTENT_PRODUCTS:
        inline_dict = category_products(request, category_id, start)
    else:
        inline_dict = category_categories(request, category_id)
...
def category_categories(request, category_id, start=0, template_name="lfs/catalog/categories/category/default.html"):
    ...
    """
    cache_key = "%s-category-categories-2-%s-%s" % (settings.CACHE_MIDDLEWARE_KEY_PREFIX, category_id,
...
def category_products(request, category_id, start=1, template_name="lfs/catalog/categories/product/default.html"):
    ...
    cache_key = "%s-category-products-2-%s-%s" % (settings.CACHE_MIDDLEWARE_KEY_PREFIX, category_id, lang)
    sub_cache_key = "%s-2-start-%s-sorting-%s-%s" % (settings.CACHE_MIDDLEWARE_KEY_PREFIX, start, sorting, lang)
...
def product_inline(request, product, template_name="lfs/catalog/products/product_inline.html"):
    ...
    cache_key = "%s-%s-product-inline-%s-%s-%s" % (settings.CACHE_MIDDLEWARE_KEY_PREFIX, group_id,
                                                   request.user.is_superuser, product.id, translation.get_language())
